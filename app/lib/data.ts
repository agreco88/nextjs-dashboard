...
export async function fetchFilteredCustomers(query: string) {
  try {
    ...
    const customers = data.rows.map(
      (customer: { total_pending: number; total_paid: number }) => ({
        ...customer,
...
export async function fetchFilteredServices(query: string) {
  try {
    const data = await sql<ServicesTable>`
      SELECT
        id,
        name,
        type,
    ...
    console.error("Database Error:", err);
    throw new Error("Failed to fetch services table.");
  }
}
